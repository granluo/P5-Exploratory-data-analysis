...
create_hist <- function(var, binw = 0.1, d = df) {
...
create_hist('volatile.acidity', binw = 0.01) + scale_x_log10(breaks = seq(0, 1, 0.1))
...
```{r}
pfree <- create_hist('free.sulfur.dioxide', 1)
ptotal <- create_hist('total.sulfur.dioxide', 1)
...
```{r}
#Add a colmn to record if a wine has SO2 effect
...
```{r}
# histogram of density, since density has very small scale, I set the width of bin to 0.0001
create_hist('density', 0.0001)
```
