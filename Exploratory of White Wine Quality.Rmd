...
From the statement of the dataset, each description of variable could be found 
...
From the statment of dataset, I found volatile acidity is the amount of acetic 
...
From the statment of dataset, I found citric acid has small quantities and can add 
...
From the statment of dataset, residual sugar is the amount of sugar remaining after 
...
In the statement of the dataset, free sulfur dioxide is the free form of SO2 exists 
...
##### pH
In this dataset, pH value describes how acidic or basic a wine is on a scale from 
...
As described in the definition above of density in this dataset, it is very close
...
In this dataset, alcohol is the percent alcohol content of the wine.
...
The main feature of this dataset is the quality. 11 properties of wines could all 
...
the dataset is factors with 7 levels from 3 to 9, due to lack of data with quality
of 0, 1, 2 and 10.
...
In this part, I will first make a matrix of plots with the dataset of white wine.
...
and alcohol has the correlations of-o.78. Total sulful dioxide and free sulfur 
...
the dataset does not have enough size of sample.
...
From the dataset, we can find wines with high quality generally have lower density,
...
```{r}
names(df)
ggplot(df,aes(x = total.sulfur.dioxide, y = free.sulfur.dioxide, color = quality.discrete))+
...
ggplot(df,aes(x = alcohol, y = density, color = quality.discrete))+
  # xlim(quantile(df$residual.sugar,0.01),quantile(df$residual.sugar,0.99))+
...
names(so2effect) <- c('Quality','No_SO2effect','no_SO2effect')
